...
The testing consists of 336 trials divided into 12 trial bundles performed from each of the 28 starting positions \todo{(see figure X)}. All movements except the rotation were blocked for the whole testing session to maintain the consistence of participant position between all participants. The trials were randomized for each participant. 

\todo{- all 28 starting locations are consistent for all participants \\
	- starting locations' order is randomized for each participant \\
	- 56 buildings, each with and without avatar = 112 target photos in total \\
	- target cannot be the starting location itself \\
	- by random choice, targets can be repeated throughout the whole experiment \\
	- a total number of 336 targets are chosen randomly from a pull of 112 targets (56 task buildings with and without avatars). Each of the four main conditions of the experiment, i.e., context meaningful with human agent presents, context meaningful with no human agent present, non-context meaningful with human agent presents, non-context meaningful with no human agent present takes 25\% of the target buildings for each participant. Target buildings are chosen from the pull of 112 targets randomly for each starting location.
} \\

Each trial is a pointing task performed from 28 different starting locations spread out through the city. At each starting point \todo{12 consecutive trials} were performed. In each trial a photo of one of the task buildings is presented at the top center of the screen. The participants have the option to press a button to bring the picture to the middle of the screen and as soon as they release it the photo moves back to the upper part of the screen. Since there is no visual virtual body, there is a green dashed laser beam attached to the virtual hand of the participants which will move as they move their hand. The purpose of the laser beam is to assist the participant with the visualization of which direction they are pointing at. The maximum duration of each trial is 30 seconds. If there is no answer given to the task, at the 20th second a countdown timer appears on the bottom center of the screen and terminates the trial after 10 seconds if there is still no answers given.
