...
Different functions of Pandas and Numpy \footnote{Python's Numpy library: \href{https://numpy.org/}{https://numpy.org/} - version 1.22.3} used for the preprocessing. The main preprocessing steps are as follow:

\begin{enumerate}
	\item Cleaning irrelevant columns.
	\item Merging the dataframe containing the information about the starting locations with the main dataframe gathered from the participants.
	\item Calculating the absolute angular error between participant's chosen direction and the actual location of the target building \footnote{In all calculations involving directions, only the right direction (x) and the forward direction (z) are taken into account.}.	
	
	\begin{enumerate}
		%\item Calculating the absolute Euclidean distance between the participant’s position and the target building. 
		\item Translating the target building's center position (Tpos) by the participant's body position (Ppos) to make the body position the origin of the following calculations.
			\begin{center}
				$Tpos_{translated}= Tpos_{x,z} - Ppos_{x,z}$
			\end{center}
